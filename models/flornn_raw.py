...
class FloRNNRaw(nn.Module):
    def __init__(self, img_channels, num_resblocks=6, num_channels=64, forward_count=2, border_ratio=0.1):
        super(FloRNNRaw, self).__init__()
        self.num_channels = num_channels
        self.forward_count = forward_count
        self.pwcnet = PWCNet()
        self.forward_rnn = ResBlocks(input_channels=img_channels + 2 + num_channels, num_resblocks=num_resblocks, num_channels=num_channels)
        self.backward_rnn = ResBlocks(input_channels=img_channels + 2 + num_channels, num_resblocks=num_resblocks, num_channels=num_channels)
    ...
    def forward(self, seqn_not_pad, a_not_pad, b_not_pad):
        ...
        a = torch.empty((N, T + self.forward_count, 1, H, W), device=a_not_pad.device)
        b = torch.empty((N, T + self.forward_count, 1, H, W), device=b_not_pad.device)
        seqn[:, :T] = seqn_not_pad
        a[:, :T] = a_not_pad
        b[:, :T] = b_not_pad
        for i in range(self.forward_count):
            seqn[:, T+i] = seqn_not_pad[:, T-2-i]
            a[:, T + i] = a_not_pad[:, T - 2 - i]
            b[:, T + i] = b_not_pad[:, T - 2 - i]
        srgb_seqn = demosaic(seqn)

        init_backward_h = torch.zeros((N, self.num_channels, H, W), device=seqn.device)
        backward_h = self.backward_rnn(torch.cat((seqn[:, 0], a[:, 0], b[:, 0], init_backward_h), dim=1))
        init_forward_h = torch.zeros((N, self.num_channels, H, W), device=seqn.device)
        forward_h = self.forward_rnn(torch.cat((seqn[:, 0], a[:, 0], b[:, 0], init_forward_h), dim=1))

        for i in range(1, T+self.forward_count):
            forward_flow = extract_flow_torch(self.pwcnet, srgb_seqn[:, i-1], srgb_seqn[:, i])
            ...
            backward_h = self.backward_rnn(torch.cat((seqn[:, i], a[:, i], b[:, i], aligned_backward_h), dim=1))
            ...
            if i >= self.forward_count:
                ...
                backward_flow = extract_flow_torch(self.pwcnet, srgb_seqn[:, i-self.forward_count+1], srgb_seqn[:, i-self.forward_count])
                aligned_forward_h, _ = warp(forward_h, backward_flow)
                forward_h = self.forward_rnn(torch.cat((seqn[:, i-self.forward_count+1], a[:, i-self.forward_count+1], b[:, i-self.forward_count+1], aligned_forward_h), dim=1))
